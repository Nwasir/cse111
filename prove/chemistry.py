...
def main():
    ...
    for inner_list in periodic_table_dict:
        name = inner_list[name_index]
        mass = inner_list[atomic_mass]
        print(f"{name}  {mass}")
    print() 
    # Get a chemical formula for a molecule from the user.
    formula = input('Enter the molecular formula of the sample: ')
    # Get the mass of a chemical sample in grams from the user.
    mass_in_grams = input('Enter the mass in grams of the sample: ')
    ...
    molarmass = mass_in_grams / atomic_mass
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for innerlist in symbol_quantity_list:

        # Separate the inner list into symbol and quantity.
         quantity = innerlist[QUANTITY_INDEX]
         symbol = innerlist[SYMBOL_INDEX]

        # Get the atomic mass for the symbol from the dictionary.
         mass = periodic_table_dict[ATOMIC_MASS_INDEX]
         atmass = mass[NAME_INDEX]

        # Multiply the atomic mass by the quantity.
         product = atmass * quantity

        # Add the product into the total molar mass.
         total_molar_mass = product + quantity
